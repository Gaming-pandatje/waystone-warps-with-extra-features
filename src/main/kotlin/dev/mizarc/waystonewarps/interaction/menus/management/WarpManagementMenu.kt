...
class WarpManagementMenu(private val player: Player, private val menuNavigator: MenuNavigator,
    ...
    override fun open() {
        ...
        // Fetch co-owners once for all permission checks
        val coOwnerIds = getCoOwners.execute(warp.id)

        // Add privacy modes
        val canChangeAccess = PermissionHelper.canChangeAccessControl(player, warp.playerId, coOwnerIds)
            ...
            if (canToggleAccess) {
                toggleLock.execute(
                    ...
                    bypassOwnership = player.hasPermission("waystonewarps.bypass.access_control")
                            || coOwnerIds.contains(player.uniqueId),
        ...
        val canManageWhitelist = PermissionHelper.canManageWhitelist(player, warp.playerId, coOwnerIds)
        ...
        // Add co-owner button (only visible to actual owner or admin)
        if (player.uniqueId == warp.playerId || player.hasPermission("waystonewarps.bypass.manage_players")) {
            val canManageCoOwners = PermissionHelper.canManageCoOwners(player, warp.playerId)
            val coOwnerItem = ItemStack(Material.EMERALD).name("Co-Owners")
            if (canManageCoOwners) {
                coOwnerItem.lore("Manage co-owners of this waystone")
            } else {
                coOwnerItem.lore("Manage co-owners of this waystone",
                    localizationProvider.get(player.uniqueId, LocalizationKeys.MENU_WARP_MANAGEMENT_COMMON_NO_PERMISSION))
            }
            val guiCoOwnerItem = GuiItem(coOwnerItem) {
                if (canManageCoOwners) {
                    menuNavigator.openMenu(WarpCoOwnerMenu(player, menuNavigator, warp, localizationProvider))
                }
            }
            pane.addItem(guiCoOwnerItem, 2, 0)
        }

        // Add renaming icon
        val canRename = PermissionHelper.canRename(player, warp.playerId, coOwnerIds)
        ...
        val canChangeIcon = PermissionHelper.canChangeIcon(player, warp.playerId, coOwnerIds)
        val iconMaterial = runCatching { Material.valueOf(warp.icon) }.getOrDefault(Material.LODESTONE)
        val iconEditorItem = ItemStack(iconMaterial).applyIconMeta(warp.iconMeta)
        ...
        // Add skins menu
        val skinMaterial = runCatching { Material.valueOf(warp.block) }.getOrDefault(Material.SMOOTH_STONE)
        val skinViewItem = ItemStack(skinMaterial)
        ...
        if (configService.warpGroupsEnabled()) {
            ...
            val canAssignGroup = player.hasPermission("waystonewarps.bypass.rename")
                    || (player.uniqueId == warp.playerId && player.hasPermission("waystonewarps.assign_group"))
        ...
        // Add move icon
        val canRelocate = PermissionHelper.canRelocate(player, warp.playerId, coOwnerIds)
